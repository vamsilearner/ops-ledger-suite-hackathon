...
export default function NextDashboard() {
  ...
  // Demo Initial State for Neha to let her experiment immediately with pre-existing operational ledger lines
  useEffect(() => {
  ...
  const saveLedgerToStorage = (newLedger: LedgerItem[]) => {
    setLedger(newLedger);
    localStorage.setItem("nep_ops_ledger", JSON.stringify(newLedger));
  };

  // Extract from raw receipt string handler
  const handleParseDocument = async () => {
    ...
    try {
      ...
      // Ensure safety and validate defaults of the returned categories
  ...
  // Save parsed and edited item to Ledger
  ...
  // Presets to let Neha test easily
...
Subtotal due and collectible: $1,500.00`);
  ...
  return (
    <div id="dashboard-root" className="min-h-screen bg-[#F8FAFC] text-slate-800 flex flex-col font-sans selection:bg-slate-200 selection:text-slate-950">
      ...
      <header id="ops-header" className="sticky top-0 z-50 bg-white border-b border-slate-200/80 px-6 py-4 backdrop-blur-md bg-white/95">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Operational Ledger Automation System • Tailored for <span className="text-slate-900 font-semibold">Neha</span>
              </p>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <div className="h-9 px-3 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold flex items-center gap-1.5 shadow-sm">
      ...
      <main id="ops-main-panel" className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6">
        ...
        <section id="insights-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
            ...
            <div className="mt-2 text-[10px] text-slate-500 font-medium">Aggregated ledger rows</div>
          </div>
          ...
          <div id="extraction-panel" className="lg:col-span-5 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="px-2.5 py-1 text-[11px] font-medium bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md border border-slate-200/80 transition-colors pointer-event-auto"
                    >
                ...
                {error && (
                  <div className="p-3 bg-rose-50 border border-rose-200 text-rose-800 text-xs rounded-xl flex items-start gap-2.5 animate-fadeIn">
                ...
                >
                  {isLoading ? (
                    <>
                      ...
                      Extracted Text structure using flash...
                    </>
                  ) : (
                    <>
            ...
            {extractedData && (
              <div id="editable-form-section" className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col gap-4 animate-slideDown">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  ...
                  <span className="text-[10px] bg-slate-100 border text-slate-600 px-2 py-0.5 rounded font-semibold tracking-wider">
                    REVIEWS MANDATORIES
                  </span>
                </div>
                ...
                <div className="flex gap-2.5 mt-2">
                  ...
                  <button
                    ...
                    className="flex-[2] py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer hover:shadow-xs"
                  >
            ...
            <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">Record of verified business expenditure transactions</p>
                </div>
              </div>

              <div className="flex items-center gap-2 self-start sm:self-auto">
                <span className="text-xs bg-slate-50 border border-slate-200/80 text-slate-700 px-3 py-1.5 rounded-lg font-semibold">
                ...
                {ledger.length > 0 && (
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm("Are you sure you want to clear Neha's entire live ledger state?")) {
                        saveLedgerToStorage([]);
                      }
                    }}
                      ...
                      <td colSpan={5} className="py-16 text-center text-slate-400">
                        <div className="flex flex-col items-center gap-2 max-w-xs mx-auto">
                          ...
                          <p className="text-[11px] text-slate-400 leading-normal">
                            No receipts parsed or listed yet. Put custom raw texts on the Intake panel on the left to start ledger sync.
                          </p>
                        </div>
                      </td>
                    </tr>
                      ...
                      <tr key={item.id} className="text-xs text-slate-700 hover:bg-slate-50/60 transition-colors group">
                        ...
                        <td className="py-3.5 px-5 text-center whitespace-nowrap">
                          <button
                            ...
                            className="p-1 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-all cursor-pointer pointer-events-auto"
      ...
      <footer id="dashboard-footer" className="bg-white border-t border-slate-200 py-5 mt-auto text-center text-[11px] text-slate-400 font-medium">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <p className="font-semibold text-slate-500">
            Secure Google Gemini 2.5 Integration
          </p>
        </div>
      </footer>
    </div>
